...
1. Given the learner has unresolved or repeated errors; when Error Notebook loads; then it shows top error patterns by frequency and recency.
2. Each pattern includes examples, affected skill, and recommended next action
3. Resolved errors are not hidden from trend calculations unless explicitly filtered
4. Empty state guidance appears for learners with no errors.
