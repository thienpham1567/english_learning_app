...
1. Given Home has at least one uncompleted plan item; when the learner clicks the primary "Start Today's Session" action; then the app opens the highest priority uncompleted plan item.
2. Completed plan items are skipped
3. If all items are complete, the CTA changes to a review or celebration state
4. The CTA remains accessible on mobile and desktop.
