...
1. Given an AI scoring or feedback route produces structured output; when the output is accepted; then the route stores template version, rubric version, model name, prompt hash, input snapshot, structured output, latency, cost estimate if available, and safety flags.
2. Malformed AI output is rejected or transformed through a safe fallback
3. Failures to persist metadata do not lose the learner's visible feedback
4. Tests cover at least writing and speaking feedback routes.

## Tasks / Subtasks

- [ ] Identify writing and speaking AI feedback routes that already produce structured output. (AC: 1-4)
- [ ] Persist `aiFeedbackRun` metadata with template/rubric/model/prompt/input/output/latency fields. (AC: 1-4)
- [ ] Add safe fallback behavior for malformed AI output or metadata persistence failure. (AC: 1-4)
- [ ] Test writing and speaking persistence, schema rejection, and visible feedback preservation. (AC: 1-4)
