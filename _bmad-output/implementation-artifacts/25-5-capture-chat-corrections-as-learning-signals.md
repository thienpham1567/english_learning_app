...
1. Given the chatbot corrects grammar, vocabulary, pronunciation, or writing issues; when a correction is structured enough to act on; then the app can save it as a learning event, error log entry, or review task candidate.
2. The learner is not interrupted by telemetry work
3. Low-confidence corrections are not forced into the review queue
4. Tests cover at least one saved grammar correction and one saved vocabulary correction.

## Tasks / Subtasks

- [ ] Define confidence thresholds for converting chat corrections into learning signals. (AC: 1-4)
- [ ] Capture structured grammar and vocabulary corrections as events, error entries, or review candidates. (AC: 1-4)
- [ ] Avoid interrupting chat streaming or user input when telemetry fails. (AC: 1-4)
- [ ] Test one grammar correction and one vocabulary correction path plus low-confidence suppression. (AC: 1-4)
