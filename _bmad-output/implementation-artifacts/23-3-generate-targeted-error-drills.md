...
1. Given an error pattern has enough source examples; when the learner starts a targeted drill; then the app generates or selects practice items that test the same pattern.
2. Every drill item includes answer validation and an explanation
3. Drill completion creates review tasks or learning events where appropriate
4. AI-generated drills are schema-validated before display.
