...
1. Given the learner completes diagnostic questions; when the diagnostic result is saved; then baseline scores are mapped to skill ids.
2. User skill states are created or updated using the baseline confidence
3. Diagnostic failure does not block normal app usage
4. Tests cover baseline creation for grammar, vocabulary, listening, and reading.
