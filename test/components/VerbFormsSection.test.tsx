...
describe("VerbFormsSection", () => {
  it("renders section header as a button with the title and item count", () => {
    ...
    expect(
      screen.getByRole("button", { name: /DẠNG ĐỘNG TỪ/i }),
    ).toBeInTheDocument();
  ...
  it("renders IPA transcriptions when expanded", async () => {
    ...
    await screen.findByText("Infinitive");
    expect(
      screen.getByText((_, el) => el?.textContent === "🇺🇸 /səˈsteɪn/"),
    ).toBeInTheDocument();
    const sustainedIpa = screen.getAllByText(
      (_, el) => el?.textContent === "🇺🇸 /səˈsteɪnd/",
    );
