/**
 * Client-safe re-exports from @repo/modules/learning.
 *
 * This file is intentionally the entry point for the `./learning` package
 * subpath export. It MUST NOT import any module that depends on @repo/database
 * or Node.js built-ins (dns, fs, net, tls, etc.) because it is bundled into
 * the browser by Next.js Client Components.
 *
 * Server-only exports (e.g. recordLearningEvent) remain available via the root
 * `@repo/modules` import which is only used in route handlers / Server Components.
 */
...
// ── record-learning-event intentionally omitted (imports @repo/database) ───
