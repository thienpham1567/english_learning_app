    ...
    <div>
      ...
      <div className="mb-6 rounded-(--radius-xl) bg-surface-alt border border-(--border) flex flex-col gap-3" style={{padding: "20px 16px"}} >
        {data.dialogue.map((line, i) => {
          ...
          return (
            <div
              key={i} className="flex flex-col" style={{alignItems: isEven ? "flex-start" : "flex-end"}} >
              {/* Speaker label */}
              <span className="text-[10px] font-extrabold uppercase tracking-widest text-text-muted mb-1" style={{paddingInline: 6}} >
                {line.speaker}
              </span>

              {/* Bubble */}
              <div className="py-2.5 px-4 leading-relaxed" style={{maxWidth: "85%", borderRadius: isMissing
                    ? "var(--radius)"
                    : isEven
                    ? "2px 14px 14px 14px"
                    : "14px 2px 14px 14px", background: isMissing
                    ? "transparent"
                    : isEven
                    ? "var(--surface)"
                    : "linear-gradient(135deg, var(--accent-light), var(--surface))", border: isMissing
                    ? "2px dashed var(--accent)"
                    : isEven
                    ? "1px solid var(--border)"
                    : "1px solid color-mix(in srgb, var(--accent) 15%, var(--border))", fontSize: 14.5, fontWeight: isMissing ? 700 : 500, color: isMissing ? "var(--accent)" : "var(--text-primary)", boxShadow: isMissing ? "none" : "var(--shadow-sm)"}} >
