...
function SectionLabel({
  ...
  return (
    <div className="flex items-center gap-2 mb-3">
      ...
      <span
        className="text-[13px] font-extrabold text-text-primary"
      ...
      {badge && (
        <span
          className="text-[10px] font-bold text-accent rounded-full"
          style={{ background: "var(--accent-light)", padding: "2px 8px", marginLeft: "auto" }}
        >
          {badge}
        </span>
      )}
...
function Chip({
  ...
  return (
    <m.button
      ...
      onClick={onClick}
      whileTap={{ scale: 0.95 }}
      className="py-2 px-4 rounded-xl cursor-pointer flex items-center relative overflow-hidden"
      style={{
        flex: "0 0 auto",
        border: active ? `1.5px solid ${color}` : "1.5px solid var(--border)",
        background: active ? `color-mix(in srgb, ${color} 12%, transparent)` : "var(--surface-alt)",
...
function CountBtn({
  ...
  return (
    <m.button
      ...
      whileTap={{ scale: 0.92 }}
      whileHover={{ y: -2 }}
      className="relative flex-1 cursor-pointer flex flex-col items-center"
      style={{
        padding: "14px 0",
        borderRadius: 14,
...
/* ════════════════════════════════════════════════
   ...
   ════════════════════════════════════════════════ */
export function PracticeSetup({ onStart }: { onStart: (params: PracticeStartParams) => void }) {
      ...
      >
        ...
        <div className="mt-4 flex justify-center gap-4 flex-wrap">
          {[
            ...
            <div
              key={s.label}
              className="flex items-center text-[11px] text-text-muted font-semibold"
              style={{ gap: 5 }}
            >
              <span
              ...
              />
              <span className="font-bold text-text-secondary">{s.label}:</span>
              <span>{s.value}</span>
            </div>
          ))}
        </div>
