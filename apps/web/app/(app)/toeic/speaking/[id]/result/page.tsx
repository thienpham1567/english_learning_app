      ...
      {metrics.alignment && (
        <div className="mt-2 text-[13px]">
          <div>
            Accuracy:{" "}
            <strong>
              {Math.round(metrics.alignment.accuracy * 100)}% ({metrics.alignment.matchedWords}/
              {metrics.alignment.expectedWords} từ)
            </strong>
          </div>
...
export default async function SpeakingResultPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [s] = await db
    ...
    .where(and(eq(toeicSpeakingSession.id, id), eq(toeicSpeakingSession.userId, session.user.id)))
    .limit(1);
  if (!s) notFound();
                  ...
                  <>
                    {r.audioPath && (
                      <audio controls className="w-full mb-2">
                        <source src={`/api/toeic-speaking/audio/${r.id}`} type="audio/webm" />
                      </audio>
                    )}
                    <div
                      className="bg-surface rounded-md italic mb-2 text-sm"
                      style={{ padding: 10 }}
                    >
                      {r.transcript ? (
                        `“${r.transcript}”`
                      ) : (
                        <em className="text-text-muted">(no transcript)</em>
                      )}
                    </div>
                    {r.feedbackVi && (
                      <div
