    ...
    <div className="mx-auto mt-8 w-full max-w-lg animate-in fade-in duration-200">
      ...
      <div className="space-y-2">
        {submissions.map((s) => (
          <button
            key={s.id}
            className="flex w-full items-center justify-between rounded-lg border-2 border-border bg-surface px-3.5 py-2.5 text-left transition hover:border-accent/40 hover:shadow-sm cursor-pointer"
            onClick={() => onView(s)}
          >
