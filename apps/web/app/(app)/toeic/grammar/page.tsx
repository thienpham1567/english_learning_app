import { db, reviewTask, toeicQuestion, userSkillState } from "@repo/database";
...
export default async function ToeicGrammarPage() {
  ...
  const skillStates = await db
    ...
    .from(userSkillState)
    .where(
      and(eq(userSkillState.userId, userId), inArray(userSkillState.skillId, PART_5_6_SKILLS)),
    );
  ...
  const poolByPart = await db
    ...
    .from(toeicQuestion)
    .innerJoin(sql`jsonb_array_elements_text(${toeicQuestion.skillIds}) AS skill`, sql`true`)
