...
export function ReviewTabs({
  ...
  const renderList = (list: ReviewQuestion[]) => {
    if (list.length === 0) {
      return (
        <div className="p-3 text-text-muted text-sm font-medium">No questions.</div>
      );
    }
    return (
          ...
          return (
            <div className="border-2 border-border rounded-xl bg-surface shadow-sm p-4" key={q.id}>
              <div className="flex gap-2 items-center mb-2.5 flex-wrap">
                ...
                {a?.flagged && <span className="bg-amber-500/10 border border-amber-500/20 text-amber-600 py-0.5 px-2 rounded-md font-bold text-xs inline-block">Bookmarked</span>}
                {a?.isCorrect === false && <span className="bg-red-500/10 border border-red-500/20 text-red-600 py-0.5 px-2 rounded-md font-bold text-xs inline-block">Incorrect</span>}
                {a?.isCorrect === true && <span className="bg-emerald-500/10 border border-emerald-500/20 text-emerald-600 py-0.5 px-2 rounded-md font-bold text-xs inline-block">Correct</span>}
                {a?.isCorrect === null && <span className="bg-accent/10 border border-accent/25 text-accent py-0.5 px-2 rounded-md font-bold text-xs inline-block">Skipped</span>}
              </div>
              {q.questionText && <div className="mb-2 font-semibold text-text-primary">{q.questionText}</div>}
