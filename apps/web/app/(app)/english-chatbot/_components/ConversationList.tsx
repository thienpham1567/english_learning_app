  ...
  return (
    <div className="relative flex h-full w-[260px] shrink-0 flex-col overflow-hidden border-r-2 border-border bg-[var(--bg)] text-text-secondary">
      ...
      <div className="relative z-10 px-4 pt-5 pb-3 space-y-3">
        ...
        <button
          onClick={handleNew}
          className="flex w-full items-center justify-center gap-2 rounded-xl border-2 border-accent/30 bg-accent/5 px-4 py-2.5 text-xs font-bold text-accent hover:bg-accent/10 hover:border-accent/50 transition-all duration-200 cursor-pointer active:scale-98"
        >
        ...
        <div className="relative">
          ...
          <input
            ...
            placeholder="Search..."
            className="w-full rounded-lg border-2 border-border bg-chat-surface pl-8 pr-3 py-2 text-[11px] font-semibold text-ink placeholder-text-muted outline-none focus:border-accent/40 transition-colors"
          />
          {search && (
            <button
              onClick={() => setSearch("")}
                ...
                {group.items.map((conv) => {
                  ...
                  if (isConfirming) {
                    return (
                      <div
                        key={conv.id}
                        className="flex items-center gap-2 rounded-xl border-2 border-error bg-error/10 p-2.5 mx-1 animate-in fade-in zoom-in-95 duration-150"
                      >
                        ...
                        <button
                          ...
                          className="flex h-6 w-6 items-center justify-center rounded-lg bg-error text-white cursor-pointer"
                        ...
                        <button
                          ...
                          className="flex h-6 w-6 items-center justify-center rounded-lg bg-chat-surface-hover text-text-secondary hover:text-ink cursor-pointer"
                  ...
                  return (
                    <div key={conv.id} className="group relative mx-1">
                      <Link
                        ...
                        className={`flex w-full items-center gap-2 rounded-xl px-3 py-2.5 text-left transition-all duration-150 ${
                          isActive
                            ? "bg-chat-surface-hover text-ink border-l-[3px] border-accent"
                            : "text-text-secondary hover:bg-chat-surface-hover/50 hover:text-text-primary"
                        }`}
                      >
                        <span className="flex-1 text-xs font-semibold leading-snug tracking-wide truncate pr-5">
