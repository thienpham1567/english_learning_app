    ...
    <div className="anim-fade-up flex flex-col gap-8">
      {STUDY_TOPICS.map((cat, catIdx) => {
        ...
        return (
          ...
          >
            {/* Category header */}
            <div className="flex items-center gap-3 mb-4 pb-3" style={{borderBottom: `2.5px solid ${allDone ? "var(--success)" : cat.color}25`}} >
              <span className="grid w-[38px] h-[38px] rounded-xl text-lg shrink-0" style={{placeItems: "center", background: allDone
                    ? "linear-gradient(135deg, var(--success), #10b981)"
                    : `linear-gradient(135deg, ${cat.color}, ${cat.color}aa)`, color: "var(--text-on-accent)", boxShadow: `0 4px 12px ${allDone ? "rgba(16, 185, 129, 0.25)" : `${cat.color}25`}`}} >
                {allDone ? <CheckCircle style={{ color: "var(--text-on-accent)" }} /> : cat.icon}
              </span>
              <div>
                <h3 className="m-0 text-base font-black text-text-primary font-display" style={{lineHeight: 1.25}} >
                  {cat.category}
                </h3>
              ...
              {cat.topics.map((topic) => {
                ...
                return (
                  <m.button
                    ...
                    whileTap={{ scale: 0.98 }} className="flex flex-col justify-between gap-3 rounded-(--radius-xl) cursor-pointer text-left" style={{padding: "16px 18px", border: "1.5px solid var(--border)", borderLeft: isDone
                        ? "5px solid var(--success)"
                        : `5px solid ${cat.color}`, background: isDone
                        ? "rgba(16, 185, 129, 0.04)"
                        : isHovered
                        ? "var(--surface-alt)"
                        : "var(--surface)", boxShadow: isHovered ? "var(--shadow-md)" : "var(--shadow-sm)", transition: "background 0.2s, border-color 0.2s"}} >
                    <div className="flex items-start justify-between gap-2.5 w-full" >
                      <span className="font-extrabold text-text-primary font-body" style={{fontSize: 14.5, lineHeight: 1.4}} >
                        {topic.title}
                      </span>
                      {isDone ? (
                        <span className="w-[20px] h-[20px] rounded-full grid shrink-0" style={{background: "var(--success)", placeItems: "center"}} >
                          <CheckCircle className="text-xs" style={{color: "var(--text-on-accent)"}} />
                        </span>
                      ) : (
                        <ChevronRight className="text-[10px] mt-1 shrink-0" style={{color: isHovered ? cat.color : "var(--text-muted)", transition: "color 0.15s"}} />
                      )}
                    </div>

                    <div className="flex items-center gap-2 flex-wrap w-full" >
                      <span className="m-0 text-[10.5px] font-black rounded-md" style={{padding: "2px 8px", background: levelStyle.bg, color: levelStyle.text, border: `1px solid ${levelStyle.border}`}} >
                        {topic.level}
                      </span>
                      <span className="text-[11px] text-text-muted flex items-center gap-1" style={{fontWeight: 650}} >
                        <Clock size={11} />
                        {topic.time}
                      </span>
