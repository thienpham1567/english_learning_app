...
export function PassageBrowser({ onSelectPassage }: PassageBrowserProps) {
  ...
  return (
    <m.div
      ...
      className="bg-surface border border-border rounded-2xl flex flex-col gap-4 p-5 shadow-sm"
    >
      ...
      <div className="flex items-center justify-between flex-wrap gap-2">
        ...
        <m.button
          ...
          disabled={aiLoading}
          className={`flex items-center gap-1.5 text-accent font-semibold uppercase tracking-wide py-1.5 px-4 text-[11px] rounded-lg border border-accent/20 bg-accent/5 shadow-sm transition-all duration-200 hover:shadow hover:bg-accent/10 ${
      ...
      <div className="flex flex-col gap-2 h-[400px] overflow-y-auto pr-1">
        {filteredPassages.length === 0 ? (
          <div className="text-center text-text-muted text-[12px] py-6 px-4 border border-dashed border-border rounded-xl">
          ...
          filteredPassages.map((sample, idx) => (
            <m.div
              ...
              className="flex items-start gap-3 border border-border rounded-xl bg-surface-alt cursor-pointer py-3 px-3.5 shadow-sm transition-all duration-200 hover:bg-accent/5 hover:border-accent/20 hover:shadow"
            >
              <div className="w-9 h-9 rounded-lg border border-border grid place-items-center text-lg shrink-0 bg-bg-deep">
                {sample.icon}
              </div>
              <div className="flex-1 min-w-0">
                <div className="font-medium text-text-primary text-[13.5px] leading-tight">
                  {sample.title}
                </div>
                ...
                <div className="flex gap-1.5 mt-1.5">
                  <span
                    className="text-[10px] font-semibold py-px px-2 rounded-full"
                    style={{
                      background:
                        sample.length === "short"
...
function FilterChip({
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className={`cursor-pointer py-1 px-3 text-[11px] whitespace-nowrap rounded-full border transition-all duration-200 ${
        active
          ? "border-accent/30 bg-accent text-text-on-accent font-semibold shadow-sm"
          : "border-border bg-surface text-text-muted font-medium hover:text-ink hover:bg-surface-hover"
      }`}
    >
      {label}
    </button>
