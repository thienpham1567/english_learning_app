      ...
      <div className="flex flex-col gap-4" >
        <m.div
            ...
            <Flex gap={8}>
              {([2, 3] as const).map((n) => (
                <button
                  key={n}
                  type="button"
                  onClick={() => setSpeakers(n)} className="flex-1 rounded-xl font-bold text-sm cursor-pointer font-body" style={{padding: "10px", border: speakers === n ? "2px solid var(--accent)" : "1px solid var(--border)", background: speakers === n ? "var(--accent-light)" : "var(--surface-alt)", color: speakers === n ? "var(--accent)" : "var(--text-secondary)"}} >
                  {n === 2 ? "👫 2 người" : "👨‍👩‍👦 3 người"}
                </button>
              ))}
            </Flex>
            ...
            <Flex gap={8}>
              {(["short", "medium", "long"] as const).map((len) => (
                <button
                  key={len}
                  type="button"
                  onClick={() => setLength(len)} className="flex-1 rounded-xl font-bold text-[13px] cursor-pointer font-body" style={{padding: "10px", border: length === len ? "2px solid var(--accent)" : "1px solid var(--border)", background: length === len ? "var(--accent-light)" : "var(--surface-alt)", color: length === len ? "var(--accent)" : "var(--text-secondary)"}} >
          ...
          <m.button
            ...
            disabled={dlg.generating} className="flex items-center justify-center gap-2.5 border-none text-[15px] font-extrabold font-body" style={{padding: "14px 20px", borderRadius: 14, background: dlg.generating ? "var(--border)" : "linear-gradient(135deg, var(--accent), var(--accent-hover))", color: dlg.generating ? "var(--text-muted)" : "var(--text-on-accent)", cursor: dlg.generating ? "wait" : "pointer", boxShadow: dlg.generating ? "none" : "0 4px 14px var(--accent-muted)"}} >
                ...
                <m.div
                  ...
                  <div className="flex shrink-0" style={{gap: 2}} >
                    {saved.voiceConfigJson.map((v) => (
                      <span key={v.speaker} className="text-base" >{v.flag}</span>
                    ))}
                  </div>

                  {/* Info */}
                  <div className="flex-1 w-[0px]" >
                    <Text className="text-[13px] font-bold text-text-primary block overflow-hidden" style={{textOverflow: "ellipsis", whiteSpace: "nowrap"}} >
                      {saved.title}
                    </Text>
                  ...
                  <button
                    type="button"
                    onClick={(e) => { e.stopPropagation(); dlg.toggleBookmark(saved.id); }} className="bg-transparent border-none text-lg cursor-pointer p-1" style={{color: saved.bookmarked ? "var(--accent)" : "var(--text-muted)", opacity: saved.bookmarked ? 1 : 0.4, transition: "all 0.15s"}} >
                    {saved.bookmarked ? "⭐" : "☆"}
                  </button>
                  ...
                  <button
                    ...
                    onMouseLeave={(e: React.MouseEvent<HTMLButtonElement>) => { e.currentTarget.style.opacity = "0.4"; }} className="bg-transparent border-none text-sm cursor-pointer p-1 text-destructive" style={{opacity: 0.4, transition: "all 0.15s"}} >
                    🗑️
                  </button>
    ...
    <div className="flex flex-col gap-4" >
      ...
      <div className="bg-(--surface) rounded-(--radius-xl) border-2 border-border py-4 px-5" style={{boxShadow: "var(--shadow-md)"}} >
        <Flex className="dialogue-header-actions" justify="space-between" align="center">
          ...
          <Flex gap={8} className="dialogue-header-buttons">
            <m.button
              whileHover={{ scale: 1.02 }}
              whileTap={{ scale: 0.98 }}
              onClick={dlg.isPlaying ? dlg.stop : () => dlg.playAll(speed)} className="flex items-center gap-1.5 py-2 px-4 rounded-xl text-[13px] font-bold cursor-pointer font-body" style={{border: dlg.isPlaying ? "1px solid var(--error)" : "1px solid var(--accent)", background: dlg.isPlaying ? "rgba(239,68,68,0.08)" : "var(--accent-light)", color: dlg.isPlaying ? "var(--error)" : "var(--accent)"}} >
        ...
        <Flex gap={8} wrap="wrap" className="dialogue-speaker-badges mt-3" >
          {dlg.voiceAssignments.map((a) => {
            ...
            return (
              <div key={a.speaker} className="flex items-center gap-2" style={{padding: "4px 12px 4px 4px", borderRadius: 10, background: colors.bg, border: `1px solid ${colors.border}`}} >
                <img
                  src={a.avatar}
                  alt={a.voiceName}
                  width={24}
                  height={24} className="rounded-lg" style={{objectFit: "cover"}} />
        ...
        {dlg.dialogue.lines.map((line, i) => {
          ...
          return (
            <m.div
              ...
              animate={{ opacity: 1, x: 0 }}
              transition={{ delay: i * 0.05 }} className="flex gap-3 items-start" style={{flexDirection: isLeft ? "row" : "row-reverse"}} >
              {/* Avatar */}
              <div className="dialogue-avatar w-[42px] h-[42px] rounded-full overflow-hidden shrink-0" style={{border: `2px solid ${colors.border}`, boxShadow: colors.shadow}} >
                <img
                  src={assignment?.avatar ?? "/avatars/austin.png"}
                  alt={assignment?.voiceName ?? "Speaker"}
                  width={42}
                  height={42} className="block" style={{objectFit: "cover"}} />
              </div>

              {/* Bubble */}
              <div className="dialogue-bubble-content relative" onClick={() => !dlg.isPlaying && dlg.playSingleLine(i, speed)} style={{maxWidth: "78%", padding: "14px 18px", borderRadius: isLeft ? "4px 18px 18px 18px" : "18px 4px 18px 18px", background: isActive
                    ? `color-mix(in srgb, ${colors.accent} 14%, var(--surface))`
                    : "var(--surface)", border: isActive
                    ? `2px solid ${colors.accent}`
                    : `1px solid ${colors.border}`, borderLeft: isLeft ? `3px solid ${colors.accent}` : undefined, borderRight: !isLeft ? `3px solid ${colors.accent}` : undefined, cursor: dlg.isPlaying ? "default" : "pointer", transition: "all 0.2s", boxShadow: isActive
                    ? `0 4px 16px color-mix(in srgb, ${colors.accent} 20%, transparent)`
                    : colors.shadow}} >
                <div className="flex items-center gap-1.5 mb-1.5" >
                  <div className="w-[6px] h-[6px] rounded-full shrink-0" style={{background: colors.accent}} />
                  <Text className="text-xs font-extrabold" style={{color: colors.text, letterSpacing: "0.02em"}} >
                    {line.name}
                  </Text>
                  <span className="text-[11px]" >{assignment?.flag ?? ""}</span>
                </div>
                <Text className="text-[15px] text-text-primary block font-medium" style={{lineHeight: 1.7}} >
                  {line.text}
                </Text>
                {isActive && dlg.isLoading && (
                  <Loader2 className="animate-spin absolute text-sm" style={{top: 10, right: 10, color: colors.accent}} />
                )}
                {isActive && !dlg.isLoading && dlg.isPlaying && (
                  <Volume2 className="absolute text-sm" style={{top: 10, right: 10, color: colors.accent}} />
                )}
              </div>
            </m.div>
          );
      ...
      {!hasListenedOnce && !rolePlaySpeaker && (
        <m.div
          ...
          animate={{ opacity: 1, y: 0 }} className="rounded-(--radius-xl) text-center" style={{background: "linear-gradient(135deg, color-mix(in srgb, var(--accent) 8%, var(--surface)), color-mix(in srgb, var(--secondary, #a78bfa) 5%, var(--surface)))", border: "2px solid color-mix(in srgb, var(--accent) 25%, var(--border))", padding: "20px", boxShadow: "0 4px 20px var(--accent-muted), var(--shadow)"}} >
          ...
          <Text className="text-[13px] text-text-muted block mb-4 w-[360px]" style={{margin: "0 auto 16px"}} >
          ...
          <Flex gap={10} justify="center" wrap="wrap" className="listen-cta-buttons">
            <m.button
              ...
              disabled={dlg.isLoading} className="flex items-center gap-2 border-none text-[15px] font-extrabold cursor-pointer font-body w-[200px] justify-center" style={{padding: "14px 28px", borderRadius: 14, background: isListeningPreview
                  ? "linear-gradient(135deg, var(--error), color-mix(in srgb, var(--error) 80%, #000))"
                  : "linear-gradient(135deg, var(--accent), var(--accent-hover))", color: "#fff", boxShadow: isListeningPreview
          ...
          {isListeningPreview && dlg.dialogue && (
            <m.div
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }} className="flex items-center justify-center gap-2" style={{marginTop: 14}} >
              <m.div
                animate={{ scale: [1, 1.4, 1], opacity: [0.5, 1, 0.5] }}
                transition={{ repeat: Infinity, duration: 1.2 }} className="w-[8px] h-[8px] rounded-full" style={{background: "var(--accent)"}} />
            ...
            {dlg.voiceAssignments.map((a) => {
              ...
              return (
                <m.button
                  ...
                  whileHover={{ scale: 1.02 }}
                  whileTap={{ scale: 0.98 }}
                  onClick={() => startRolePlay(a.speaker)} className="flex-1 flex items-center justify-center gap-2 py-3 px-4 rounded-xl text-sm font-bold cursor-pointer font-body" style={{border: `1px solid ${SPEAKER_COLORS[a.speaker]?.border ?? "var(--border)"}`, background: SPEAKER_COLORS[a.speaker]?.bg ?? "var(--surface-alt)", color: SPEAKER_COLORS[a.speaker]?.text ?? "var(--text-primary)"}} >
      ...
      <AnimatePresence>
        {rolePlaySpeaker && (
          <m.div
            initial={{ opacity: 0, y: 10 }}
            animate={{ opacity: 1, y: 0 }}
            exit={{ opacity: 0 }} className="bg-(--surface) rounded-(--radius-xl) py-4 px-5 flex flex-col gap-3" style={{border: "2px solid var(--accent)", boxShadow: "0 4px 20px var(--accent-muted), var(--shadow-md)"}} >
            ...
            {rolePlayStep === "idle" && (
              <m.button
                whileHover={{ scale: 1.02 }}
                whileTap={{ scale: 0.98 }}
                onClick={() => playRolePlayLine(0)} className="w-full p-3 rounded-xl border-none text-sm font-extrabold cursor-pointer font-body" style={{background: "linear-gradient(135deg, var(--accent), var(--accent-hover))", color: "var(--text-on-accent)"}} >
                ▶ Bắt đầu hội thoại
              </m.button>
            )}
            ...
            {rolePlayStep === "listening" && (
              <Flex align="center" justify="center" gap={8} className="p-3" >
                <m.div animate={{ scale: [1, 1.2, 1] }} transition={{ repeat: Infinity, duration: 1.5 }} className="w-[12px] h-[12px] rounded-full" style={{background: "var(--accent)"}} />
            ...
            {rolePlayStep === "recording" && (
              <Flex vertical gap={8} align="center" className="p-3" >
                <Flex align="center" gap={8}>
                  <m.div animate={{ scale: [1, 1.3, 1], opacity: [1, 0.5, 1] }} transition={{ repeat: Infinity, duration: 1 }} className="w-[14px] h-[14px] rounded-full" style={{background: "var(--error)"}} />
                ...
                <m.button
                  whileHover={{ scale: 1.02 }}
                  whileTap={{ scale: 0.98 }}
                  onClick={stopAndEvaluateRolePlay} className="flex items-center gap-2 rounded-xl text-destructive text-sm font-extrabold cursor-pointer font-body" style={{padding: "10px 24px", border: "2px solid var(--error)", background: "rgba(239,68,68,0.08)"}} >
            ...
            {rolePlayStep === "result" && rolePlayResult && (
              <>
                ...
                <m.button
                  whileHover={{ scale: 1.02 }}
                  whileTap={{ scale: 0.98 }}
                  onClick={continueAfterRolePlay} className="w-full p-3 rounded-xl border-none text-sm font-extrabold cursor-pointer font-body" style={{background: "linear-gradient(135deg, var(--accent), var(--accent-hover))", color: "var(--text-on-accent)"}} >
