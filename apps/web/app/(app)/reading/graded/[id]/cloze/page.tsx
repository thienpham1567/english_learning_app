    ...
    <div className="h-full overflow-y-auto" style={{padding: "var(--space-6)"}} >
      ...
      <Flex vertical gap={20} className="w-[720px] mx-auto" >
        ...
        <Button
          type="text"
          icon={<ArrowLeft />}
          onClick={() => router.push(`/reading/graded/${id}`)} className="text-text-muted text-[13px]" style={{alignSelf: "flex-start", borderRadius: 10}} >
          Quay lại bài đọc
        </Button>
        ...
        <Card
          ...
          styles={{ body: { padding: "20px 24px" } }} className="border-none" style={{borderRadius: 20, background: "linear-gradient(135deg, var(--accent), var(--secondary))"}} >
          <Flex align="center" gap={14}>
            <div className="w-[44px] h-[44px] rounded-xl flex items-center justify-center" style={{background: "rgba(255,255,255,0.2)"}} >
              <Pencil size={22} className="text-(--text-on-accent)" />
            </div>
            <div>
              <Text className="text-[11px] uppercase" style={{letterSpacing: "0.1em", color: "rgba(255,255,255,0.7)"}} >
                CLOZE TEST
              </Text>
            ...
            <Tag className="border-none font-bold text-[13px]" style={{marginLeft: "auto", borderRadius: 10, background: "rgba(255,255,255,0.2)", color: "var(--text-on-accent)", padding: "4px 14px"}} >
              {totalCount} câu
            </Tag>
          </Flex>
        </Card>
        ...
        {submitted && (
          <Card
            ...
            styles={{ body: { padding: "24px" } }} className="text-center" style={{borderRadius: 20, border: `2px solid ${score >= 80 ? "var(--success)" : score >= 50 ? "var(--warning)" : "var(--error)"}`, background: score >= 80 ? "color-mix(in srgb, var(--success) 3%, transparent)" : score >= 50 ? "color-mix(in srgb, var(--warning) 3%, transparent)" : "color-mix(in srgb, var(--error) 3%, transparent)"}} >
            <div className="w-[56px] h-[56px] rounded-2xl flex items-center justify-center" style={{margin: "0 auto 12px", background: score >= 80 ? "color-mix(in srgb, var(--success) 12%, transparent)" : score >= 50 ? "color-mix(in srgb, var(--warning) 12%, transparent)" : "color-mix(in srgb, var(--error) 12%, transparent)"}} >
              {score >= 80
          ...
          {items.map((item, i) => {
            ...
            return (
              <Card
                ...
                styles={{ body: { padding: "14px 20px" } }} className="rounded-2xl" style={{border: showResult
                    ? `1.5px solid ${isCorrect ? "var(--success)" : "var(--error)"}`
                    : "1px solid var(--border)", background: showResult
                    ? isCorrect ? "var(--success)06" : "var(--error)06"
                    : undefined, transition: "all 0.2s ease"}} >
                <Flex gap={10} align="flex-start">
                  {/* Number badge */}
                  <div className="w-[28px] h-[28px] rounded-lg flex items-center justify-center text-xs font-bold shrink-0" style={{background: showResult
                      ? isCorrect ? "color-mix(in srgb, var(--success) 8%, transparent)" : "color-mix(in srgb, var(--error) 8%, transparent)"
                      : "var(--surface)", marginTop: 2, color: showResult
                  ...
                  <div className="flex-1" >
                    <Text className="text-sm" style={{lineHeight: 1.8}} >
                      ...
                      <input
                        ...
                        }} className="rounded-lg bg-(--surface) text-sm font-semibold text-center" style={{width: Math.max(90, item.answer.length * 12), padding: "3px 10px", border: showResult
                            ? `2px solid ${isCorrect ? "var(--success)" : "var(--error)"}`
                            : "1.5px dashed var(--accent)", outline: "none", color: showResult
                            ? isCorrect ? "var(--success)" : "var(--error)"
                            : "var(--text)", transition: "border-color 0.2s"}} />
                      {" "}{item.after}
                    </Text>
        ...
        {!submitted && (
          <Button
            ...
            size="large"
            icon={<CheckCircle />}
            onClick={handleSubmit} className="rounded-xl font-bold h-[48px] text-[15px]" style={{alignSelf: "center", padding: "0 32px"}} >
