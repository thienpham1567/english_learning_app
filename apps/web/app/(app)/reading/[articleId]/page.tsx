    ...
    return (
      <div className="flex flex-col h-full items-center justify-center gap-4 p-8" >
        ...
        <m.button
          ...
          whileTap={{ scale: 0.98 }} className="rounded-lg border-none font-extrabold cursor-pointer" style={{padding: "8px 24px", background: "var(--accent)", color: "var(--text-on-accent)"}} >
          Quay lại danh sách
        </m.button>
      </div>
    );
    ...
    <div className="relative flex h-full h-[0px] flex-1 flex-col overflow-hidden" >
      ...
      <div className="relative h-[0px] flex-1 overflow-y-auto z-[1]" style={{padding: "24px 20px 80px"}} >
        ...
        <div className="w-[720px] mx-auto flex flex-col gap-5" >
          {/* Header Action Menu */}
          <div className="flex items-center justify-between" style={{borderBottom: "1px solid var(--border)", paddingBottom: 14}} >
            <m.button
              onClick={() => router.push("/reading")}
            ...
            <span className="text-[11px] font-black rounded-md text-accent uppercase tracking-wider" style={{padding: "4px 10px", background: "var(--accent-light)", border: "1.5px solid var(--accent-muted)"}} >
          ...
          <div className="flex flex-col gap-2.5" >
            <h2 className="font-display text-text-primary m-0" style={{fontSize: 26, fontWeight: 950, lineHeight: 1.3}} >
              {article.title}
            </h2>
            ...
            <div className="flex gap-3 flex-wrap mt-1.5 pb-4" style={{alignContent: "center", borderBottom: "1px dashed var(--border)"}} >
              {article.author && (
              ...
              <div className="items-center gap-1 text-text-muted font-bold" style={{display: "inline-flex", fontSize: 12.5}} >
                <Clock />
                <span>{article.readTime} phút đọc</span>
              </div>
              <span style={{ color: "var(--border-strong)" }}>|</span>
              <span className="text-[11px] font-extrabold rounded-md" style={{padding: "2px 8px", background: diffStyle.bg, color: diffStyle.color, border: `1.5px solid ${diffStyle.border}`}} >
          ...
          {article.thumbnail && (
            ...
            <img
              ...
              alt={article.title} className="w-full h-[340px] rounded-(--radius-xl)" style={{objectFit: "cover", border: "1.5px solid var(--border)"}} />
          )}

          {/* Reader Body Content */}
          <div className="font-medium text-text-primary flex flex-col gap-5" style={{fontSize: 16.5, lineHeight: 1.85, letterSpacing: "0.01em"}} >
            {article.paragraphs.map((para, idx) => (
              <div
                key={idx} className="relative" style={{paddingRight: 60}} >
                <p className="m-0" style={{textAlign: "justify"}} >{para}</p>

                {/* Floating control buttons per paragraph */}
                <div className="absolute flex flex-col gap-1.5" style={{right: 0, top: "50%", transform: "translateY(-50%)"}} >
                  ...
                  <m.button
                    ...
                    title="Phân tích ngữ pháp" className="w-[26px] h-[26px] rounded-full border border-(--border) flex items-center justify-center" style={{background: grammarResults[idx]?.length
                        ...
                        ? "var(--text-on-accent)"
                        : "var(--text-muted)", cursor: grammarLoading.has(idx) ? "wait" : "pointer"}} >
          ...
          >
            {grammarPopup !== null && grammarResults[grammarPopup] && (
              <div>
                {/* Header banner */}
                <div className="py-4 px-5 flex items-center gap-2" style={{background: "linear-gradient(135deg, var(--accent), var(--secondary))", color: "var(--text-on-accent)"}} >
                  <Lightbulb size={16} />
                  <span className="font-black" style={{fontSize: 14.5}} >
                    Phân tích ngữ pháp của đoạn văn
                  </span>
                  <span className="rounded-xl text-[10.5px] font-extrabold" style={{background: "rgba(255,255,255,0.2)", padding: "2px 8px"}} >
                    {grammarResults[grammarPopup].length} cấu trúc
                  </span>
                </div>

                {/* Body details list */}
                <div className="flex flex-col gap-3 h-[380px] overflow-y-auto bg-(--surface)" style={{padding: 20}} >
                  {grammarResults[grammarPopup].map((pattern, pi) => (
                    <div
                      key={pi} className="rounded-lg bg-surface-alt" style={{borderLeft: `4.5px solid ${PATTERN_COLORS[pattern.color] ?? "var(--border)"}`, borderTop: "1px solid var(--border)", borderRight: "1px solid var(--border)", borderBottom: "1px solid var(--border)", padding: "12px 14px"}} >
                      <span className="font-black block mb-1.5" style={{fontSize: 13.5, color: PATTERN_COLORS[pattern.color] ?? "var(--accent)"}} >
                        {pattern.name}
                      </span>
