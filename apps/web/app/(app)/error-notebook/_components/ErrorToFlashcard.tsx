    ...
    <div>
      ...
      {state === "idle" && (
        <m.button
          ...
          whileTap={{ scale: 0.98 }}
          onClick={generate}
          className="w-full flex items-center justify-center gap-2 py-3 px-4 rounded-xl border-2 border-accent/30 bg-accent/5 text-accent text-xs font-extrabold cursor-pointer transition-colors hover:bg-accent/10"
        >
          <CreditCard size={14} />
      ...
      <AnimatePresence>
        {state === "preview" && card && (
          <m.div
            ...
            exit={{ opacity: 0, y: -8 }}
            className="mt-3 rounded-xl border-2 border-accent/20 bg-surface overflow-hidden"
          >
            {/* Card Header */}
            ...
            <div className="px-4 py-3 border-t-2 border-border flex gap-2">
              <m.button
                ...
                whileTap={{ scale: 0.98 }}
                onClick={generate}
                className="flex-1 flex items-center justify-center gap-1.5 py-2.5 px-4 rounded-xl border-2 border-border bg-surface text-text-secondary text-xs font-bold cursor-pointer transition-colors hover:bg-surface-alt"
              >
                <Sparkles size={12} />
                Regenerate
              </m.button>
