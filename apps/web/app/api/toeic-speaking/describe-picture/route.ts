...
export async function POST(request: Request) {
  ...
  if (entry && entry.resetAt > now) {
    if (entry.count >= RATE_LIMIT_MAX) return Response.json({ error: "Rate limit exceeded." }, { status: 429 });
    entry.count++;
  } else {
  ...
  if (!(audioFile instanceof File)) return Response.json({ error: "No audio file" }, { status: 400 });
  ...
  try { keyElements = JSON.parse(coerceString(formData.get("keyElements"), "[]")); } catch { /* empty */ }
  ...
  if (!transcription.ok) return Response.json({ error: transcription.error }, { status: transcription.status });
  ...
  if (!transcript || transcript.trim().split(/\s+/).filter(Boolean).length < MIN_WORDS) {
    return Response.json({ error: "no-speech", message: "Không nhận dạng được giọng nói." }, { status: 422 });
  }
...
Check if the speaker mentioned the key elements. Be encouraging but honest.`;
  ...
  try {
    ...
    return Response.json({ error: aborted ? "Evaluation timed out" : "Failed to evaluate" }, { status: aborted ? 504 : 502 });
  } finally {
