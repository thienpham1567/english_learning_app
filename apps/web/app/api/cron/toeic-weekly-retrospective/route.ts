...
export async function GET(request: Request) {
  ...
  const activeUsers = await db
    ...
    .from(learningEvent)
    .where(and(gte(learningEvent.createdAt, since), like(learningEvent.moduleType, "toeic_%")))
  ...
  for (const { userId } of activeUsers) {
    const [events, skillStates, dueTasks, errors] = await Promise.all([
      ...
      db
        ...
        .from(userSkillState)
        .where(
          and(eq(userSkillState.userId, userId), sql`${userSkillState.skillId} LIKE 'toeic.%'`),
        ),
      db
