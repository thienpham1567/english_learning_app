 ...
 */
export function computeFluency(words: WhisperWord[], durationMs: number): PronunciationMetrics {
  ...
  const lowConfidenceWords = Array.from(
    new Set(
      cleanWords.filter((w) => (w.probability ?? 1) < LOW_CONFIDENCE_THRESHOLD).map((w) => w.word),
    ),
...
export function computeAlignment(
  ...
  const expected = expectedText
    ...
    .map(normalizeToken)
    .filter((t) => t.length > 0);
  const spoken = spokenWords.map((w) => normalizeToken(w.word)).filter((t) => t.length > 0);
  ...
  for (let i = 0; i < m; i++) {
    for (let j = 0; j < n; j++) {
      dp[i + 1][j + 1] =
        expected[i] === spoken[j] ? dp[i][j] + 1 : Math.max(dp[i + 1][j], dp[i][j + 1]);
    }
  }
