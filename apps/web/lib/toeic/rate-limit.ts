 ...
 */
import { db, toeicAttempt, toeicSpeakingSession, toeicWritingSession } from "@repo/database";
import { and, eq, gte, sql } from "drizzle-orm";

export type RateLimitedAction = "mock_test" | "speaking_session" | "writing_session";
...
export async function assertRateLimit(
  ...
  if (action === "mock_test") {
    ...
    const [r] = await db
      ...
      .where(
        and(eq(toeicSpeakingSession.userId, userId), gte(toeicSpeakingSession.startedAt, since)),
      );
    usedToday = r?.c ?? 0;
    ...
    const [r] = await db
      ...
      .where(
        and(eq(toeicWritingSession.userId, userId), gte(toeicWritingSession.startedAt, since)),
      );
    usedToday = r?.c ?? 0;
  }
