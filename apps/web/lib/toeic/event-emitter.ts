...
import { produceReviewTask, recordLearningEvent } from "@repo/modules";
...
export async function emitToeicLearningEvent(input: EmitToeicEventInput): Promise<void> {
  const result = input.isCorrect === null ? "neutral" : input.isCorrect ? "correct" : "incorrect";
  ...
  const difficulty = (
    ["beginner", "elementary", "intermediate", "upper_intermediate", "advanced"].includes(
      ...
      ? input.difficulty
      : "intermediate"
  ) as "beginner" | "elementary" | "intermediate" | "upper_intermediate" | "advanced";
