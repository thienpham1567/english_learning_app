...
export function buildDictionaryInstructions(entryType: DictionaryEntryType) {
  return [
    ...
    // ── Sense: examples ───────────────────────────────────────────────────
    "Provide 2 to 3 examples per sense. Each example is a bilingual object with `en` (English sentence) and `vi` (natural Vietnamese translation). The set of examples for a sense must collectively show variety: at minimum two different syntactic patterns or contexts (e.g. one with a subject + verb usage, one as part of a larger clause; one neutral, one with a common collocation).",
