...
describe("DictionarySearchPanel", () => {
  ...
  it("shows autocomplete suggestions after debounce when value has 2+ chars", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByRole("option", { name: /take off/ }),
      ).toBeInTheDocument();
      expect(
        screen.getByRole("option", { name: /take on/ }),
      ).toBeInTheDocument();
    });
  });
  ...
  it("ArrowDown highlights the first suggestion", async () => {
    ...
    fireEvent.keyDown(getByPlaceholderText("Ví dụ: take off"), {
      key: "ArrowDown",
    });
  ...
  it("Escape clears suggestions", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.queryByRole("option", { name: /take off/ }),
